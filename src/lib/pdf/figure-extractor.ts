...
export async function extractFigures(
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (let i = 0; i < analysis.figures.length; i++) {
      ...
      const figureLabel = fig.caption?.match(/^((?:Figure|Table|Fig\.?)\s*\d+)/i)?.[1] || `page${page}-fig${i}`;

      const record: ExtractedFigure = {
        pdfPage: page,
        figureIndex: i,
        type: fig.type,
        captionText: fig.caption,
      ...
      await prisma.paperFigure.upsert({
        where: {
          paperId_sourceMethod_figureLabel: { paperId, sourceMethod: "pdf_embedded", figureLabel },
        },
        create: {
          paperId,
          ...
          type: fig.type,
          captionText: fig.caption,
          captionSource: fig.caption ? "vision_llm" : "none",
          description: fig.description,
          imagePath,
          width: dims.width,
          height: dims.height,
...
export async function saveFiguresToMindPalace(
  ...
  for (const fig of figures) {
    ...
    const learning = fig.captionText
      ? `${fig.captionText}\n\n${fig.description}`
      : fig.description;
    ...
    try {
      ...
      await prisma.insight.create({
        data: {
          ...
          significance,
          applications: `See page ${fig.pdfPage} of the paper. Image: ${fig.imagePath}`,
          isAutoGenerated: true,
          source: "distill",
        },
      });
