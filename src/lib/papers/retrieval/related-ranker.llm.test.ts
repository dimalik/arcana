...
vi.mock("./open-reranker", () => ({
  runOpenRelatedReranker: hoisted.runOpenRelatedReranker,
}));

vi.mock("../../llm/prompts", () => ({
  SYSTEM_PROMPTS: {
    rerankRelatedPapers: "rank related papers",
    scoreRelatedPapersPointwise: "score related papers pointwise",
  },
}));
...
describe("related-ranker llm backend", () => {
  beforeEach(() => {
    ...
    hoisted.runOpenRelatedReranker.mockResolvedValue(new Map());
  });
  ...
  it("uses pointwise assessments to keep only genuinely related candidates", async () => {
    hoisted.generateStructuredObject.mockResolvedValue({
      object: {
        assessments: [
          {
            ...
            include: true,
            relevanceScore: 0.93,
            rationale: "Directly targets efficient attention for LLM inference.",
            primarySignals: ["shared-problem", "efficient-attention"],
            exclusionSignals: [],
          },
          {
            ...
            include: true,
            relevanceScore: 0.88,
            rationale: "Addresses the same serving bottleneck through memory-aware LLM inference.",
            primarySignals: ["llm-serving", "deployment-setting"],
            exclusionSignals: [],
          },
          {
            paperId: "good-c",
            include: true,
            relevanceScore: 0.81,
            rationale: "Focuses on batched efficient attention for LLM serving.",
            primarySignals: ["efficient-attention", "llm-serving"],
            exclusionSignals: [],
          },
          {
            ...
            include: false,
            relevanceScore: 0.12,
            rationale: "This is vision diffusion work and only overlaps at a broad model buzzword level.",
            primarySignals: [],
            exclusionSignals: ["generic-adjacency", "topic-mismatch"],
          },
        ],
        summary: "Kept the efficient inference cluster and excluded the vision diffusion near miss.",
      },
    });
    ...
    expect(result.rerankedRows.some((row) => row.relatedPaper.id === "bad-c")).toBe(
      false,
    );
  });

  it("falls back to the feature backend if pointwise reranking fails", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    hoisted.generateStructuredObject.mockRejectedValue(
      new Error("provider unavailable"),
    );
  ...
  it("uses an open reranker backend to refine the shortlist", async () => {
    hoisted.runOpenRelatedReranker.mockResolvedValue(
    ...
    const featureResult = await buildRelatedRerankResult(
      "seed",
      "user-1",
      relationRows,
      createMockDb() as never,
    ...
    const featureGoodC = featureResult.diagnostics.find(
      (diagnostic) => diagnostic.paperId === "good-c",
    );
    const rerankedGoodC = result.diagnostics.find(
      (diagnostic) => diagnostic.paperId === "good-c",
    );
    expect(featureGoodC?.rerankScore ?? 0).toBeLessThan(rerankedGoodC?.rerankScore ?? 0);
    expect(hoisted.runOpenRelatedReranker).toHaveBeenCalledTimes(1);
  });

  it("falls back to the feature backend if the open reranker fails", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    hoisted.runOpenRelatedReranker.mockRejectedValue(
      new Error("python backend unavailable"),
    );
