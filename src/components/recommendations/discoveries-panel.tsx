...
import { RefreshCw, Plus, Check, BookOpen } from "lucide-react";
...
function PaperCard({
  ...
  return (
    <div className="group relative p-2.5 rounded-lg border border-border/30 hover:border-border/60 hover:bg-accent/30 transition-colors">
      <a
      ...
      {paper.abstract && (
        <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-200">
          <div className="overflow-hidden">
            <p className="text-xs text-muted-foreground/80 mt-1.5 leading-relaxed line-clamp-4">
              {paper.abstract}
            </p>
          </div>
        </div>
...
export function DiscoveriesPanel({ tagIds = [] }: DiscoveriesPanelProps) {
  ...
  const hasResults = data && ((data.latest?.length ?? 0) > 0 || (data.recommended?.length ?? 0) > 0);
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <h3 className="text-sm font-medium">Discoveries</h3>
          {tagIds.length > 0 && (
            <span className="text-[10px] font-medium rounded-full bg-primary/10 text-primary px-1.5 py-0.5">
              filtered
            </span>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-8 text-center rounded-lg border border-dashed border-border/40">
          ...
          <p className="text-xs text-muted-foreground/60 px-4">
            Add papers to get personalized recommendations
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {(data.latest?.length ?? 0) > 0 && (
            <Section title="Latest in your field">
